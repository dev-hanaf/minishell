...
void should_expand(char *line, int *j,char **str,  int to_expand, t_env *env)
{
    int i = *j;
    if (line[i] && to_expand && line[i] == '$' && line[i + 1] && line[i + 1] == '?')
    {
        char *exp = ft_itoa(g_minishell.status);
        line = ft_strjoin(ft_substr(line, 0, i), exp);
        line = ft_strjoin(line, ft_substr(line, i + 2, ft_strlen(line) - i - 2));
    }
}
...
char **catch_expand(char *line, t_env *env, int to_expand)
{
    ...
    if (line &&  ft_strchr(line, '$'))
    {
        while(line[i])
        {
            ...
            should_expand(line, &i, str, to_expand, env);
            if (line[i] && to_expand  && line[i] == '$'  &&  line[i + 1] && (ft_isalnum(line[i + 1]) || line[i  + 1] == '_' ))
            {
                ...
                if (space && !open)
                {
                    ...
                    str[exp_cmpt] = ft_strjoin(str[exp_cmpt], exp);
                continue;
            }
            ...
            else if (line[i] && i > 0 && line[i] == '$' && line[i - 1] == '"' && line[i + 1] == '"')
            {
                usleep(1);
                //TODO pass();
            }
